...
class BbendigobankPipeline:

    # Database setup
    conn = sqlite3.connect('bendigobank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bendigobank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bendigobank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bendigobank (date, title, link, content)"
